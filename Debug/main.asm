...
;	::->op : IFX
	...
	JMP	_00010_DS_
;	::->op :*  =
	.line	65; "../main.c"	T2IF =0;
	BANKSEL	_EIF1_bits
	CLR	_EIF1_bits, 1
;	::->op : LABEL
;	::->op : GET_VALUE_AT_ADDRESS
_00010_DS_
	.line	67; "../main.c"	if(T1IF & T1IE)
	BANKSEL	r0x1005
	CLR	r0x1005
	BANKSEL	_EIF1_bits
	JB	_EIF1_bits, 0
...
;	::->op : IFX
	...
	JMP	_00064_DS_
;	::->op :*  =
	.line	69; "../main.c"	T1IF =0;
	BANKSEL	_EIF1_bits
	CLR	_EIF1_bits, 0
;	::->op : IFX
	.line	70; "../main.c"	if(PWM_Timer_Flag)
	MOV	R0,# 0x00
	...
	JMP	_00060_DS_
;	::->op : =
	.line	72; "../main.c"	PWM_Timer_Flag =0;
	BANKSEL	_PWM_Timer_Flag
	CLR	_PWM_Timer_Flag
;	::->op : =
	.line	73; "../main.c"	T1H = 0xF1;
	MOV	R0,# 0xf1
	BANKSEL	_T1H
	MOV	_T1H, R0
;	::->op : =
	.line	74; "../main.c"	T1L = 0x60;
	MOV	R0,# 0x60
	BANKSEL	_T1L
	MOV	_T1L, R0
;	::->op : BITWISEAND
	.line	75; "../main.c"	if(Stop_High_Addr & LED1ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 0
	JMP	_00012_DS_
;	::->op :*  =
	.line	76; "../main.c"	LED1LOW;
	BANKSEL	_P2LR_bits
	CLR	_P2LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00012_DS_
	.line	77; "../main.c"	if(Stop_High_Addr & LED2ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 1
	JMP	_00014_DS_
;	::->op :*  =
	.line	78; "../main.c"	LED2LOW;
	BANKSEL	_P2LR_bits
	CLR	_P2LR_bits, 4
;	::->op : LABEL
;	::->op : BITWISEAND
_00014_DS_
	.line	79; "../main.c"	if(Stop_High_Addr & LED3ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 2
	JMP	_00016_DS_
;	::->op :*  =
	.line	80; "../main.c"	LED3LOW;
	BANKSEL	_P2LR_bits
	CLR	_P2LR_bits, 0
;	::->op : LABEL
;	::->op : BITWISEAND
_00016_DS_
	.line	81; "../main.c"	if(Stop_High_Addr & LED4ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 3
	JMP	_00018_DS_
;	::->op :*  =
	.line	82; "../main.c"	LED4LOW;
	BANKSEL	_P2LR_bits
	CLR	_P2LR_bits, 1
;	::->op : LABEL
;	::->op : BITWISEAND
_00018_DS_
	.line	83; "../main.c"	if(Stop_High_Addr & LED5ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 4
	JMP	_00020_DS_
;	::->op :*  =
	.line	84; "../main.c"	LED5LOW;
	BANKSEL	_P6LR_bits
	CLR	_P6LR_bits, 1
;	::->op : LABEL
;	::->op : BITWISEAND
_00020_DS_
	.line	85; "../main.c"	if(Stop_High_Addr & LED6ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 5
	JMP	_00022_DS_
;	::->op :*  =
	.line	86; "../main.c"	LED6LOW;
	BANKSEL	_P6LR_bits
	CLR	_P6LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00022_DS_
	.line	87; "../main.c"	if(Stop_High_Addr & LED7ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 6
	JMP	_00024_DS_
;	::->op :*  =
	.line	88; "../main.c"	LED7LOW;
	BANKSEL	_P6LR_bits
	CLR	_P6LR_bits, 5
;	::->op : LABEL
;	::->op : BITWISEAND
_00024_DS_
	.line	89; "../main.c"	if(Stop_High_Addr & LED8ADDR)
	BANKSEL	_Stop_High_Addr
	JB	_Stop_High_Addr, 7
	JMP	_00026_DS_
;	::->op :*  =
	.line	90; "../main.c"	LED8LOW;
	BANKSEL	_P6LR_bits
	CLR	_P6LR_bits, 6
;	::->op : LABEL
;	::->op : BITWISEAND
_00026_DS_
	.line	91; "../main.c"	if(Stop_High_Addr & LED9ADDR)
	BANKSEL	(_Stop_High_Addr + 1)
	JB	(_Stop_High_Addr + 1), 0
	JMP	_00028_DS_
;	::->op :*  =
	.line	92; "../main.c"	LED9LOW;
	BANKSEL	_P9LR_bits
	CLR	_P9LR_bits, 5
;	::->op : LABEL
;	::->op : BITWISEAND
_00028_DS_
	.line	93; "../main.c"	if(Stop_High_Addr & LED10ADDR)
	BANKSEL	(_Stop_High_Addr + 1)
	JB	(_Stop_High_Addr + 1), 1
	JMP	_00030_DS_
;	::->op :*  =
	.line	94; "../main.c"	LED10LOW;
	BANKSEL	_P9LR_bits
	CLR	_P9LR_bits, 4
;	::->op : LABEL
;	::->op : BITWISEAND
_00030_DS_
	.line	95; "../main.c"	if(Stop_High_Addr & LED11ADDR)
	BANKSEL	(_Stop_High_Addr + 1)
	JB	(_Stop_High_Addr + 1), 2
	JMP	_00032_DS_
;	::->op :*  =
	.line	96; "../main.c"	LED11LOW;
	BANKSEL	_P4LR_bits
	CLR	_P4LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00032_DS_
	.line	97; "../main.c"	if(Stop_High_Addr & LED12ADDR)
	BANKSEL	(_Stop_High_Addr + 1)
	JB	(_Stop_High_Addr + 1), 3
	JMP	_00064_DS_
;	::->op :*  =
	.line	98; "../main.c"	LED12LOW;
	BANKSEL	_P4LR_bits
	CLR	_P4LR_bits, 5
;	::->op : GOTO
	JMP	_00064_DS_
...
;	::->op : =
_00060_DS_
	.line	102; "../main.c"	PWM_Timer_Flag =1;
	MOV	R0,# 0x01
	BANKSEL	_PWM_Timer_Flag
	MOV	_PWM_Timer_Flag, R0
;	::->op : =
	.line	103; "../main.c"	T1H = 0xFF;
	MOV	R0,# 0xff
	BANKSEL	_T1H
	MOV	_T1H, R0
;	::->op : =
	.line	104; "../main.c"	T1L = 98;
	MOV	R0,# 0x62
	BANKSEL	_T1L
	MOV	_T1L, R0
;	::->op : BITWISEAND
	.line	105; "../main.c"	if(Stop_Low_Addr & LED1ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 0
	JMP	_00036_DS_
;	::->op :*  =
	.line	106; "../main.c"	LED1HIGH;
	BANKSEL	_P2LR_bits
	SET	_P2LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00036_DS_
	.line	107; "../main.c"	if(Stop_Low_Addr & LED2ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 1
	JMP	_00038_DS_
;	::->op :*  =
	.line	108; "../main.c"	LED2HIGH;
	BANKSEL	_P2LR_bits
	SET	_P2LR_bits, 4
;	::->op : LABEL
;	::->op : BITWISEAND
_00038_DS_
	.line	109; "../main.c"	if(Stop_Low_Addr & LED3ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 2
	JMP	_00040_DS_
;	::->op :*  =
	.line	110; "../main.c"	LED3HIGH;
	BANKSEL	_P2LR_bits
	SET	_P2LR_bits, 0
;	::->op : LABEL
;	::->op : BITWISEAND
_00040_DS_
	.line	111; "../main.c"	if(Stop_Low_Addr & LED4ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 3
	JMP	_00042_DS_
;	::->op :*  =
	.line	112; "../main.c"	LED4HIGH;
	BANKSEL	_P2LR_bits
	SET	_P2LR_bits, 1
;	::->op : LABEL
;	::->op : BITWISEAND
_00042_DS_
	.line	113; "../main.c"	if(Stop_Low_Addr & LED5ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 4
	JMP	_00044_DS_
;	::->op :*  =
	.line	114; "../main.c"	LED5HIGH;
	BANKSEL	_P6LR_bits
	SET	_P6LR_bits, 1
;	::->op : LABEL
;	::->op : BITWISEAND
_00044_DS_
	.line	115; "../main.c"	if(Stop_Low_Addr & LED6ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 5
	JMP	_00046_DS_
;	::->op :*  =
	.line	116; "../main.c"	LED6HIGH;
	BANKSEL	_P6LR_bits
	SET	_P6LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00046_DS_
	.line	117; "../main.c"	if(Stop_Low_Addr & LED7ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 6
	JMP	_00048_DS_
;	::->op :*  =
	.line	118; "../main.c"	LED7HIGH;
	BANKSEL	_P6LR_bits
	SET	_P6LR_bits, 5
;	::->op : LABEL
;	::->op : BITWISEAND
_00048_DS_
	.line	119; "../main.c"	if(Stop_Low_Addr & LED8ADDR)
	BANKSEL	_Stop_Low_Addr
	JB	_Stop_Low_Addr, 7
	JMP	_00050_DS_
;	::->op :*  =
	.line	120; "../main.c"	LED8HIGH;
	BANKSEL	_P6LR_bits
	SET	_P6LR_bits, 6
;	::->op : LABEL
;	::->op : BITWISEAND
_00050_DS_
	.line	121; "../main.c"	if(Stop_Low_Addr & LED9ADDR)
	BANKSEL	(_Stop_Low_Addr + 1)
	JB	(_Stop_Low_Addr + 1), 0
	JMP	_00052_DS_
;	::->op :*  =
	.line	122; "../main.c"	LED9HIGH;
	BANKSEL	_P9LR_bits
	SET	_P9LR_bits, 5
;	::->op : LABEL
;	::->op : BITWISEAND
_00052_DS_
	.line	123; "../main.c"	if(Stop_Low_Addr & LED10ADDR)
	BANKSEL	(_Stop_Low_Addr + 1)
	JB	(_Stop_Low_Addr + 1), 1
	JMP	_00054_DS_
;	::->op :*  =
	.line	124; "../main.c"	LED10HIGH;
	BANKSEL	_P9LR_bits
	SET	_P9LR_bits, 4
;	::->op : LABEL
;	::->op : BITWISEAND
_00054_DS_
	.line	125; "../main.c"	if(Stop_Low_Addr & LED11ADDR)
	BANKSEL	(_Stop_Low_Addr + 1)
	JB	(_Stop_Low_Addr + 1), 2
	JMP	_00056_DS_
;	::->op :*  =
	.line	126; "../main.c"	LED11HIGH;
	BANKSEL	_P4LR_bits
	SET	_P4LR_bits, 3
;	::->op : LABEL
;	::->op : BITWISEAND
_00056_DS_
	.line	127; "../main.c"	if(Stop_Low_Addr & LED12ADDR)
	BANKSEL	(_Stop_Low_Addr + 1)
	JB	(_Stop_Low_Addr + 1), 3
	JMP	_00064_DS_
;	::->op :*  =
	.line	128; "../main.c"	LED12HIGH;
	BANKSEL	_P4LR_bits
	SET	_P4LR_bits, 5
;	::->op : LABEL
;	::->op : ENDFUNCTION
...
;	::->op : =
	...
	MOV	_SCLKCTL, R0
;	::->op : =
	.line	28; "../main.c"	HFCKCTL =0x8E;//使能高频外设时钟，INTHF/64,
	MOV	R0,# 0x8e
	BANKSEL	_HFCKCTL
	MOV	_HFCKCTL, R0
;	::->op : CALL
	.line	29; "../main.c"	Init_IO();
	TRAPPC1	_Init_IO
	TRAPPC2	_Init_IO
	PAGESEL	_Init_IO
	CALL	_Init_IO
	...
	PAGESEL	$
;	::->op : CALL
	.line	30; "../main.c"	IS31FL3265B_Init();
...
;	::->op : CALL
	...
	PAGESEL	$
;	::->op : =
	.line	34; "../main.c"	Stop_High_Addr = 0xFFFF;
	MOV	R0,# 0xff
	BANKSEL	_Stop_High_Addr
...
;	::->op : =
	.line	35; "../main.c"	Stop_Low_Addr = 0xFFFF;
	MOV	R0,# 0xff
	BANKSEL	_Stop_Low_Addr
...
;	::->op :*  =
	.line	36; "../main.c"	PUIE =1;
	BANKSEL	_INTCTL_bits
	SET	_INTCTL_bits, 6
;	::->op :*  =
	.line	37; "../main.c"	AIE =1;
	BANKSEL	_INTCTL_bits
	SET	_INTCTL_bits, 7
;	::->op : LABEL
;	::->op : CALL
_00002_DS_
	.line	40; "../main.c"	RT_Check_Input();
	...
	PAGESEL	$
;	::->op : CALL
	.line	41; "../main.c"	Tail_Stop_Check_Input();
...
;	  211+   96 =   307 instructions (  806 byte)

	.type _main,  32
	.dim _main, 1, (_main_function_end&0xFF00)>>8, _main_function_end&0xFF,0,25,0,54 , 1, 1
	.type _int_fun0,  32
	.dim _int_fun0, 1, (_int_fun0_function_end&0xFF00)>>8, _int_fun0_function_end&0xFF,0,56,0,131 , 2, 1
	.type _int_fun1,  32
	.dim _int_fun1, 1, (_int_fun1_function_end&0xFF00)>>8, _int_fun1_function_end&0xFF,0,135,0,138 , 2, 1
